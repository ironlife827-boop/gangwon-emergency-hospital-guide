...
- 진료과 수: 18개
- 의심 질환 수: 121개
- 질환별 문진 질문: 126개 질환, 532개 문항 확보
- `naver_crawl_query_plan.csv` 기준 추가 수집 후보 450건 중 432건을 필터 통과 데이터로 반영
- 감기, 독감, 코로나19, 편도염, 기관지염, 위염, 방광염 등 상시 병원 안내 질환 라벨을 추가 반영
...
12. 최종 응급도 1~2단계는 응급실/병상 중심, 3~5단계는 주변 일반 병원/의원 중심으로 추천
...
- 감기/독감/코로나19 같은 흔한 호흡기 질환이 기도폐쇄나 아나필락시스로 오분류되지 않도록 상시 질환 커버리지 보강
- 비응급 감기 케이스에서 최종 5단계와 주변 일반 병원 추천 흐름을 회귀 테스트로 고정
